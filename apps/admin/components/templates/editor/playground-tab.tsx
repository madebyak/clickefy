...
import type { Template, TemplateInput, GenerationStage } from '@clickfy/types';
...
export function PlaygroundTab({ template }: PlaygroundTabProps) {
  ...
  const runStage = useCallback(
    async (
      ...
      // Build the exact payload the API route expects — a
      // `CompileContext`-shaped body. The server compiles + executes
      // via @clickfy/providers, so the playground exercises the same
      // pipeline mobile jobs do.
      //
      // `inputValues` mirrors `RuntimeInputValue`: text inputs go as
      // `{ kind: 'text', value }`, image/video uploads as
      // `{ kind: 'image' | 'video', mimeType, base64 }` (the server
      // re-hydrates base64 → Uint8Array).
      const inputValues: Record<string, unknown> = {};
      for (const [fieldKey, data] of Object.entries(testInputs)) {
        if (data.type === 'text') {
          inputValues[fieldKey] = { kind: 'text', value: data.value };
        } else if (data.base64) {
      ...
      // Convert the playground's running tally of prior outputs to
      // the `StageOutputRef[]` shape the compiler consumes.
      const previousOutputRefs = previousOutputs.map((output, i) => ({
        stageIndex: i + 1,
        kind: output.type,
      ...
      const response = await fetch('/api/generate', {
        ...
        body: JSON.stringify({
          ...
          inputValues,
          previousOutputs: previousOutputRefs,
        }),
      });
                        ...
                        <input
                          ...
                          accept={
                            ('acceptedFormats' in input && input.acceptedFormats?.length
                              ...
                              : input.type === 'image' || input.type === 'image_multi'
                                ? 'image/*'
                                : 'video/*')
                          }
                          onChange={(e) => {
              ...
              <div className="grid grid-cols-1 gap-3">
                {testRun.outputs.map((output, i) => (
                  <Card key={i} className="overflow-hidden group">
                    {output.type === 'image' ? (
                      // Centre the image on a neutral backdrop and let it
                      // render at its native aspect ratio. The previous
                      // `aspect-square object-cover` cropped portrait /
                      // wide outputs into 1:1 thumbnails which made it
                      // look like the model was ignoring the aspect
                      // ratio config — it wasn't, we were just lying
                      // about the size on screen.
                      <div className="relative flex items-center justify-center bg-muted/30 max-h-[640px] overflow-hidden">
                        <img
