/**
 * Legal pack content — WEB COPY.
 *
 * ⚠️ SOURCE OF TRUTH: apps/mobile/lib/legal-content.ts — the exact text
 * users see inside the mobile app. Keep the two files in lockstep: any
 * substantive edit must land in BOTH and bump EFFECTIVE_DATE in both.
 * (The doc bodies below are byte-identical to the mobile file.)
 *
 * Rendered at /terms, /privacy, /ai-disclosure, /content-policy, /dmca —
 * publicly, with no auth — because Google Play and Apple review require
 * externally reachable legal pages (docs/WEB_LEGAL_HANDOFF.md).
 *
 * Content is deliberately English-only on both locales: the English text
 * is the legally reviewed version; the page chrome is localized.
 */
